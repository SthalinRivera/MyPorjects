<template>
    <div class="min-h-screen ">
        ...
        <div class="max-w-6xl mx-auto">
            ...
            <!-- Generated Matrix -->
            <div v-if="apiResponse && stage === 'completed'" class="mt-8 max-w-6xl mx-auto m-2">
                ...
                <MatrizConsistenciaTable v-if="apiResponse && stage === 'completed'" :api-response="apiResponse" />
...
import SkeletonTableMatriz from '~/components/UI/Skeleton/SkeletonTableMatriz.vue';
...
const improveTitle = async () => {
    ...
    try {
        ...
        if (!loggedIn.value) {
            ...
            localStorage.setItem('freeGenerationCount', generationCount.value);
...
// Modificar el botón de generación para deshabilitarlo cuando se alcance el límite
const isGenerateDisabled = computed(() => {
    return stage.value !== 'idle' ||
        !inputValue.value.trim() ||
        (!loggedIn.value && generationCount.value >= maxFreeGenerations);
});
// Métodos actualizados
const improveTitlev2 = async () => {
    if (!inputValue.value.trim()) {
        $toast.error("Por favor ingrese un tema de investigación");
        return;
    }
...
// Mostrar el badge flotante después de 5 segundos
onMounted(() => {
    setTimeout(() => {
        showFloatingBadge.value = true
        // Mostrar modal principal solo la primera vez
    ...
    // Ocultar el mensaje principal después de 8 segundos si no está logueado
    setTimeout(() => {
        if (!loggedIn.value) {
